...
async function iniciarUsuario() {
    ...
    if (!userId || !userData) {
        ...
        try {
            await fetchFromSupabase('users', 'POST', currentUser);
    ...
    if (container && messages.length === 0) {
        container.innerHTML = `
            <div class="welcome-message">
                ...
                <p style="margin-top: 16px; background: rgba(0,0,0,0.3); padding: 10px; border-radius: 8px;">
                    <strong>TU ID:</strong><br>
                    <code style="font-size: 11px;">${currentUser.id}</code>
                </p>
            </div>
        `;
    }
...
window.agregarAmigo = async function(friendId) {
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/rest/v1/friends?user_id=eq.${currentUser.id}&friend_id=eq.${friendId}`, {
            headers: {
                'apikey': SUPABASE_ANON_KEY,
                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`
            }
    ...
    try {
        ...
        if (amigos && amigos.length > 0) {
            ...
            amigos.forEach(amigo => {
                ...
                let avatarHtml = (amigo.avatar_type === 'image' && amigo.avatar) ? 
                    '<img src="' + amigo.avatar + '" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">' : 
                    (amigo.avatar || '👤');
...
function renderMessages() {
    ...
    if (messages.length === 0) {
        if (!container.innerHTML.includes('TU ID')) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('searchSubmitBtn').onclick = async () => {
        ...
        if (usuario) {
            let avatarHtml = (usuario.avatar_type === 'image' && usuario.avatar) ? 
                '<img src="' + usuario.avatar + '" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover;">' : 
