...
async function iniciarUsuario() {
    ...
    if (!userId || !userData) {
        ...
        if (error) console.error('Error creando usuario:', error);
...
async function cargarAmigos() {
    ...
    if (amigos) {
        ...
        amigos.forEach(amigo => {
            ...
            let avatarHtml = '';
            if (amigo.avatarType === 'image' && amigo.avatar) {
...
async function sendPrivateMessage(text, imageUrl) {
    ...
    const { error } = await supabase.from('private_messages').insert([mensaje]);
...
async function editarPerfil(nuevoNombre, fotoData) {
    ...
    const { error } = await supabase
        .from('users')
        .update({ name: currentUser.name, avatar: currentUser.avatar, avatarType: currentUser.avatarType })
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Botones modales
    document.getElementById('searchUserBtn').onclick = () => searchModal.style.display = 'flex';
    document.getElementById('addFriendBtn').onclick = () => addFriendModal.style.display = 'flex';
    document.getElementById('editProfileBtn').onclick = () => editProfileModal.style.display = 'flex';
    ...
    document.getElementById('searchSubmitBtn').onclick = async () => {
        ...
        if (usuario) {
            let avatarHtml = '';
            if (usuario.avatarType === 'image' && usuario.avatar) {
            ...
            resultDiv.innerHTML = '<div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">' +
                avatarHtml +
                '<div><strong>' + escapeHtml(usuario.name) + '</strong><br><small>ID: ' + usuario.id + '</small></div>' +
    ...
    photoInput.addEventListener('change', (e) => {
        ...
        if (file && file.type.startsWith('image/')) {
            ...
            reader.onload = (ev) => {
                ...
                photoPreview.innerHTML = '<img src="' + tempPhotoData + '" style="width: 80px; height: 80px; border-radius: 50%; object-fit: cover;">';
            };
    ...
    document.getElementById('saveProfileBtn').onclick = async () => {
        ...
        await editarPerfil(newName, tempPhotoData);
